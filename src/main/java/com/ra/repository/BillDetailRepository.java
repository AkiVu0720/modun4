...
public interface BillDetailRepository extends JpaRepository<BillDetailModel,Integer> {
    ...
    @Query("select sum(b.importPrice*b.quantity) from BillDetailModel b where b.bill.created = month (current_date)")
    Double sumInDay();
    @Query("select sum(b.importPrice*b.quantity) from BillDetailModel b")
    double sumInMonth();
    @Query("select sum(b.importPrice*b.quantity) from BillDetailModel b ")
    double sumInYear();
}
